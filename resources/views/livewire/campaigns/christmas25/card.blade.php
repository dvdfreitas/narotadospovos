...
mount(function ($code) {
    ...
    // Lemos o modo do postal via query string: ?view=donor|gift
...
@php
    ...
    // Dados “de oferta” vivem no JSON campaign_data
    ...
    // Nome a mostrar ao destinatário (se o doador for anónimo)
    $donorDisplay = $donation->is_anonymous ? 'um/a amigo/a' : $donation->donor_name;

    // Normalizamos view para evitar valores estranhos
    $view = in_array($view, ['donor','gift'], true) ? $view : 'gift';

    // Só mostramos “gift view” se for mesmo uma doação-oferta.
    // Caso contrário, fazemos fallback para donor.
    ...
    $itemMap = [
        ...
        'custom'  => ['emoji' => '🎁', 'label' => 'Prenda Solidária'],
    ];
    ...
    <div class="min-h-screen py-8 md:py-12 flex items-center justify-center bg-gray-100 px-4 font-sans">
        <div class="airmail-border max-w-5xl w-full transform -rotate-1 hover:rotate-0 transition-transform duration-700 ease-out">
            <div class="postcard-paper w-full h-full min-h-[550px] relative grid md:grid-cols-2 shadow-inner">
                ...
                <div class="p-8 md:p-14 flex flex-col justify-center relative z-10">
                    <div class="font-handwriting text-2xl md:text-[1.6rem] text-blue-900 leading-loose">
                        ...
                        @if ($isGiftView)
                            ...
                            <p class="mb-6">
                                O/A <span class="text-emerald-700 font-bold decoration-clone bg-emerald-50/80 px-1 rounded">{{ $donorDisplay }}</span>
                                decidiu oferecer-te uma <strong>prenda solidária</strong> em teu nome, apoiando as crianças da Casa da Mamé.
                            </p>

                            @if ($giftMessage)
                                <div class="mb-6 relative">
                                    <span class="text-4xl text-blue-200 absolute -top-4 -left-2">"</span>
                                    <p class="text-gray-600 italic px-4">{{ $giftMessage }}</p>
                            ...
                            <div class="mt-6 text-xs text-gray-500 font-sans">
                                Este postal não contém dados fiscais nem informação de pagamento.
                            </div>

                        @else
                            {{-- === DONOR CARD (postal do doador) === --}}
                ...
                {{-- RIGHT SIDE: HEADER & STAMP & ADDRESS --}}
                <div class="p-8 md:p-12 flex flex-col relative z-10">

                    <div class="flex justify-between items-start mb-16">
                        <div class="pt-2 opacity-30">
                            <h1 class="font-sans font-bold text-gray-400 tracking-[0.35em] text-lg uppercase select-none">
                                Postal Solidário
                            </h1>
                        </div>
                        ...
                        <div class="flex items-center gap-6 relative">
                            ...
                            {{-- Stamp: diferente consoante o modo --}}
                            <div class="postage-stamp p-1.5 shadow-md rotate-2">
                                <div class="w-[85px] h-[100px] bg-emerald-50 flex flex-col items-center justify-center border border-dotted border-gray-300 relative overflow-hidden">
                                    ...
                                    <img src="/images/funding/christmas2025/tree.png" class="h-10 w-auto object-contain mb-2 drop-shadow-sm" alt="Árvore de Natal">
                                    ...
                                    @else
                                        {{-- GIFT: não mostra valor --}}
                                        <span class="text-[10px] font-bold text-emerald-900 border-t border-emerald-200 pt-1 w-full text-center uppercase tracking-wider">
                                            Prenda Solidária
                                        </span>
                                    @endif
                                </div>
                    ...
                    {{-- ADDRESS LINES (diferentes no gift card) --}}
                    <div class="mt-auto space-y-8 font-typewriter text-gray-800 text-xl pl-4 md:pl-8">
                        ...
                        @if ($isGiftView)
                            <div class="border-b border-gray-300 pb-1 relative">
                                <span class="text-[10px] text-gray-400 absolute -top-4 left-0 font-sans uppercase tracking-widest">Para:</span>
                                {{ $recipientName ?: '—' }}
                            </div>

                            <div class="border-b border-gray-300 pb-1 relative">
                                <span class="text-[10px] text-gray-400 absolute -top-4 left-0 font-sans uppercase tracking-widest">De:</span>
                                {{ $donorDisplay }}
                            </div>

                            <div class="border-b border-gray-300 pb-1 flex justify-between items-baseline">
                                <span>Na Rota dos Povos</span>
                                <span class="text-xs font-sans text-gray-400 uppercase tracking-widest">Natal</span>
                            </div>
                        @else
                            <div class="border-b border-gray-300 pb-1 relative">
                        ...
                        <div class="pt-6 flex justify-end">
                            <div class="bg-yellow-100/80 text-yellow-800 text-xs font-mono px-3 py-1.5 rounded-sm shadow-sm border border-yellow-200 -rotate-2">
                                Donativo <span class="font-bold text-base tracking-widest">#{{ $donation->access_code }}</span>
                            </div>
                        </div>
                    </div>
