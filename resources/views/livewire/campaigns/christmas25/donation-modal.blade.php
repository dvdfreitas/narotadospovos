...
rules([
    ...
    // REMOVIDO: regra do email destinatário
...
on(['open-donation-modal' => function ($type = 'papas', $amount = 5) {
    ...
    $this->reset([
        'donor_name', 'donor_email', 'donor_phone', 'nif',
        'public_message', 'gift_recipient_name', // Removed email reset
        'gift_message', 'createdDonation', 'terms'
    ]);
    ...
    // --- LAZY TESTING MODE (Auto-fill in local env) ---
...
// Injection of the correct service class
$save = function (IfThenPayService $paymentService) {
    ...
    $donation = Donation::create([
        ...
        // REMOVIDO: gift_recipient_email da base de dados (passar null se a coluna existir, ou remover a linha)
    ...
    try {
        // 2. Call the Service (Returns MbWayResponse DTO)
        $result = $paymentService->requestMbWayPayment(
            $this->donor_phone,
            $this->amount,
            (string) $donation->id,
            'Natal Solidario #' . $donation->id
        );

        // 3. Check Success using the new helper method
        if ($result->isSuccess()) {

            $donation->update([
                'payment_status' => 'paid',
                'payment_gateway_id' => $result->requestId
            ]);
                ...
                @if($step === 1)
                    ...
                    <form wire:submit="save">
                        <div class="px-6 py-6 space-y-6">
                            ...
                            {{-- Products --}}
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-3">O que queres oferecer?</label>
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                <div class="space-y-3">
                                    ...
                                    <input type="email" wire:model="donor_email" placeholder="Email (para o recibo)" class="w-full text-sm rounded-lg border-neutral-300">
                                    ...
                                    <div class="bg-neutral-50 p-3 rounded-lg border border-neutral-200">
                                        ...
                                        @if($is_gift)
                                            <div class="space-y-2 animate-in fade-in">
                                                ...
                                                {{-- REMOVIDO: Email input --}}

                                                <textarea wire:model="gift_message" rows="2" placeholder="Mensagem para o postal..." class="w-full text-xs rounded border-neutral-300"></textarea>
                            ...
                            {{-- NOVO: Privacy Checkbox --}}
                            <div class="pt-2 border-t border-gray-100">
                                <label class="flex items-start gap-2 cursor-pointer">
                                    <input type="checkbox" wire:model="terms" class="mt-1 rounded text-emerald-600 border-neutral-300 focus:ring-emerald-500">
                                    <div class="text-xs text-neutral-500">
                                        Aceito a
                                        <a href="{{ route('christmas.privacy') }}" target="_blank" class="font-bold text-emerald-700 hover:underline">
                                ...
                                @error('terms') <span class="text-xs text-red-500 font-bold block mt-1 ml-6">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-xl shadow-sm hover:border-blue-300 transition-colors">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="text-left">
                                        ...
                                        <p class="text-sm font-bold text-blue-900">O teu Recibo / Postal</p>
                                    </div>
                                </div>
                            ...
                            @if($createdDonation->is_gift)
                                <div class="relative overflow-hidden flex items-center justify-between p-4 bg-emerald-50 border border-emerald-200 rounded-xl shadow-sm hover:border-emerald-300 transition-colors group">
                                    ...
                                    <div class="relative z-10 flex items-center gap-4">
                                        ...
                                        <div class="text-left">
                                            <p class="text-[10px] font-bold tracking-widest text-emerald-600 uppercase">Prenda para</p>
                                ...
                                <p class="text-[10px] text-center text-neutral-400 mt-2">
                                    Copia o link acima e envia por WhatsApp.
                                </p>
                            @endif
                        </div>
