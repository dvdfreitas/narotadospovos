<x-appLayout>
    ...
    xl:mx-20 2xl:max-w-full transition-all duration-75 ease-in-out">
        <form method="POST" action="/stories" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-flow-row gap-6 lg:grid lg:grid-cols-2 border-b border-gray-900/10 
            pb-8 transition-all duration-75 ease-in-out">
                ...
                <div class="col-span-1 space-y-8 sm:space-y-6">
                    <fieldset class="space-y-2">
                        ...
                        <div>
                            <x-input id="title" class="block mt-1 w-full" type="text" name="title" value="{{old('title')}}" placeholder="Título" required autofocus />
                    ...
                    <div class="flex flex-col space-y-6 md:flex md:flex-row md:space-y-0 md:space-x-6 
                    transition-all duration-75 ease-in-out">
                        <fieldset class="w-full space-y-2 md:w-1/2">
                            <x-label>Data</x-label>
                            <div>
                                <x-input type="date" name="date" value="{{ old('date') }}" id="date" class="w-full" placeholder="dd-mm-aa" />
                                <x-input-error for="date" />
                            </div>
                        </fieldset>
                        <fieldset class="w-full space-y-2 md:w-1/2">
                            <x-label for="categories">Categoria:</x-label>
                            <select name="category_id[]" id="categories" class="block w-full space-y-2 rounded-md border-0
                    ...
                    <fieldset class="space-y-2">
                        ...
                        <p class="text-sm leading-6 text-gray-600">Escreva a discrição da notícia a publicar.</p>
                    </fieldset>
                </div>
            </div>
